...
def linear_to_srgb_torch(linear: torch.Tensor,
    ...
    thresh = torch.tensor(0.0031308)
    srgb0 = 323 / 25 * linear
    srgb1 = (211 * torch.maximum(eps, linear)**(5 / 12) - 11) / 200
    return torch.where(linear <= thresh, srgb0, srgb1)
...
def srgb_to_linear_torch(srgb: torch.Tensor,
                   eps: Optional[float] = None) -> torch.Tensor:
    """Assumes `srgb` is in [0, 1], see https://en.wikipedia.org/wiki/SRGB."""
    if eps is None:
        eps = torch.finfo(torch.float32).eps
